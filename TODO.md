...
**`forge sync --langs`:**
- Scan `~/.forge/langs/default/<each>/lang.wl` (via symlink) and `~/.forge/langs/custom/<each>/lang.wl`
- Read each `lang.wl`, extract fields
- Write `~/.forge/langs.json`
- User-created langs in `custom/` are discovered automatically (no store involvement)

**`forge sync --includes`:**
- Scan `~/.forge/includes/default/<each>/` and `~/.forge/includes/custom/<each>/`
- Read `include.wl` and `setup.sh` for each
- Write `~/.forge/includes.json`

**`forge create` / `forge lang`:** read `langs.json` to get available languages, parse their `lang_wl` content to get defaults.
